...
def test_logme_perfect_features(
    dummy_features_perfect, dummy_labels, logme_minimum_datatpoints_required
):
    """Test that LogME returns a value greater than zero for perfect features, with less
    than zero only seen for random features.

    Empirically at least 3500 datapoints are required for a representative score."""
    features = np.concatenate(
        [
            np.asarray(dummy_features_perfect)
            for _ in range(
                (
                    logme_minimum_datatpoints_required
                    // dummy_features_perfect.shape[0]
                    + 1
                )
            )
        ],
    ...
    labels = np.concatenate(
        [
            ...
            for _ in range(
                (logme_minimum_datatpoints_required // dummy_labels.shape[0] + 1)
            )
        ],
        axis=1,
    )
...
def test_logme_random_features(
    dummy_features_random, dummy_labels, logme_minimum_datatpoints_required
):
    ...
    features = np.concatenate(
        [
            ...
            for _ in range(
                (
                    logme_minimum_datatpoints_required // dummy_features_random.shape[0]
                    + 1
                )
            )
        ],
