...
def test_renggli_class_random_features(
    ...
    assert metric.fit_metric(
        dummy_features_random, dummy_labels, random_state=test_seed
...
def test_parc_class_perfect_features(dummy_features_perfect, dummy_labels, test_seed):
    ...
    assert metric.dataset_dependent is True
    assert metric.fit_metric(
        dummy_features_perfect, dummy_labels, random_sate=test_seed
    ) == pytest.approx(100)
...
def test_parc_class_random_features(dummy_features_random, dummy_labels, test_seed):
    ...
    assert metric.dataset_dependent is True
    assert metric.fit_metric(
        dummy_features_random, dummy_labels, random_sate=test_seed
